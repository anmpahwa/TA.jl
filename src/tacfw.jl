"""
    tacfw(G::Graph, tol::Float64, maxiters::Int64, maxruntime::Int64, log::Symbol)

Conjugate Frank-Wolfe method for static multi-class traffic assignment problem with generalized link cost function.
...
"""
function tacfw(G::Graph, tol::Float64, maxiters::Int64, maxruntime::Int64, log::Symbol)
    ...
    while true
        ...
        elseif iszero(den) λ = 0.0
        else λ = 0.99
        end
        for (k,a) ∈ enumerate(A) p[k] = λ .* pₙ₋₁[k] + (1 - λ) .* yₙ[k] end
